...
(def twitter-streams [who-stream mentions-stream number-stream])

(defn start-streams [] (doseq [s twitter-streams] (client/start-twitter-stream s)))
    ...
    (let [who (rand-nth candidates)
      ...
      (statuses-update :oauth-creds my-creds
                       ...
                       :callbacks (SyncSingleCallback. (partial reset-state who)
                                                       ...
                                                       exception-rethrow)))
    (if (> (:minutes-since-update @state) (+ mike-jones-interval 5))
      (do
        (log/debug (str "no mike jones candidates found after " (:minutes-since-update @state) " minutes, restarting streams"))
        (swap! state #(assoc % :minutes-since-update mike-jones-interval))
        (doseq [s twitter-streams] (client/cancel-twitter-stream s))
        (start-streams))
...
(defn get-mention-reply
  [tweetMap]
  (let [name (get-in tweetMap [:user :screen_name])]
...
(defn reply-to-follows
  [user-events]
  (when-let [follows (seq (->> user-events
                               (filter #(= (:event %) "follow"))))]
